...
def dist(x0s, y0s, scalxy, lam, x00, y00, rot, disp0, a3=0, a5=0):
    ...
    the function also allows to fit a sqrt for the radial function, approximated
    by 
...
def errorsum(params, maxs, maxl, x, y, lam):
    ...
    for s in range(0, maxs):
        for l in range(0, maxl):
            if x[s, l] > 0:
                (xf[s, l], yf[s, l]) = dist(x0[s], y0[s], scalxy, lam[s, l], x00, y00, rot, disp0, a3, a5)
    # calculate errors
    xf = xf - x
    yf = yf - y
...
def lsqf(parv, debug=False, fit_report=False):
    ...
    params['a3'].vary = True
    if typesqrt: params['a5'].vary = False
    else:  params['a5'].vary = True
    time0 = time.time()
    #------------------------------------------------------------------------------
    # LEAST SQUARE FIT
    #------------------------------------------------------------------------------
    out = minimize(errorsum, params, args=args)
    if fit_report: report_fit(out)
    #------------------------------------------------------------------------------
    ...
    #------------------------------------------------------------------------------
    if debug: print('parameters after fit:')
    result = f'{comment}\n{outfil}.txt\n'
    result += 'Parameters after fit:\n'
    result += f"a3     ={a3:12.4e}\n"
    result += f"a5     ={a5:12.4e}\n"
    for i in range(5):   
        if debug: print(f"{parstr[i]:6s} ={par[i]:12.4f}")
...
def calib_setup(ini_file, par_dict, res_dict, fits_dict, opt_dict, logtext):
    ...
    wloc_setup = (opt_dict['win_x'] + opt_dict['setup_off_x'], opt_dict['win_y'] + opt_dict['setup_off_y'])
